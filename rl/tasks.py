...
class TaskCueReward:
    def __init__(self, s,do_shuffle=True): 
    ...
    def create_csus_vectors(self):
        ...
        for ic in np.arange(self.nb_cs):
            ...
            n_tr_cue = int(np.ceil(percent_per_cs[ic]*n_trials)[0])
            cs_vector.append(np.int8(ic*np.ones((1,n_tr_cue))).flatten())
            rv=[]
            for ir in np.arange(len(rews)):
                ntr_rw = int( np.ceil(prs[ir]*n_tr_cue))
                rv.append(rews[ir]*np.ones( (ntr_rw,1)).flatten())
            rv = np.concatenate(rv)[0:n_tr_cue]
            id_perm= np.arange(len(rv))
            np.random.shuffle(id_perm)
            us_vector.append(rv[id_perm])
...
#             rews = np.asarray(mags[ic])
#             prs = np.asarray(probs[ic])
#             n_tr_cue = np.int8(np.ceil(percent_per_cs[ic]*n_trials)[0])
#             cs_vector.append(np.int8(ic*np.ones((1,n_tr_cue))).flatten())
#             rv=[]
#             for ir in np.arange(len(rews)):
#                 ntr_rw = np.int8( np.ceil(prs[ir]*n_tr_cue))
#                 rv.append(rews[ir]*np.ones( (ntr_rw,1)).flatten())
#             rv = np.concatenate(rv)[0:n_tr_cue]
#             us_vector.append(rv)
#         us_vector = np.concatenate(us_vector) 
