"""
@author: jakd@lundbeck.com  2018
modified by Sandra Romero Pinto 2022 sromeropinto@g.harvard.edu
...
class DA:
    """
    This is a dopamine class. 
    Sets up a set of eqns that represents dopamine levels in midbrain and Nucleus accumbens / Dorsal striatum. 
    Parameters depend on area. 
    The somatodendritic compartment uses the same parameters. 
    
    :param area: The location of cell bodies can be either 'vta' for mesolimbic projections or 
                'SNc' for nigrostriatal projections. 
                Default is 'vta'. Not case sensitive. 
    :type area: str
    ...
    **Examples:**
    ...
    .. Note::
        ...
        - In the above example *Conc* is a two element array. 2nd element is drug concentraion. 
            First element is a placeholder replaced with current DA concentraion by the DA.update-method
        - If all receptor occupancies of the DA-cllass will be vectors if the instance is created with drug argument.
            Note that occupancy is two element: First is DA occupany, second is drug occupancy. *activity* is scalar. 
    ...
    """ MicMen reuptake parameter. Affected by cocaine or methylphenidate. 
        If running multiple systems, they can all be updated using ``DA.Km = xxx`` """
    ...
    Precurser = 1.0; 
    """This is a parameter that is used to represent the DA release capacity. 
    If *precurser* = 1, then DA release is normal. Use ``myDA.precurser = 3.0`` to simulate effect of L-dopa"""
    ...
    def __init__(self, area = "VTA", *drugs):
        ...
        self._drugs = drugs   
        # self.D2term = TerminalFeedback(1., k_on_term, k_off_term, D2occupancyTerm, efficacy)
        ...
        self.area = area
        if area.lower() == "vta":
            self.Vmax_pr_neuron = 10 #1500.0/Original_NNeurons # VALUES FFROM WHERE? 
            self.Gamma_pr_neuron = 200.0/Original_NNeurons 
            #This value is deliberately high and anticipates a ~50% reduction by terminal feedback
        elif area.lower() == "snc":
        ...
        self.Conc_DA_term = 20.0;# SANDRA : WHY? 50.0;
    ...
    def update(self, dt, nu_in = 5, e_stim = False, Conc = np.array([0.0])):
        ...
        :type dt: float
        :param nu_in: Input firing rate used in the update in Hz. Default is 5 Hz. This is *before* somadodendritic autoinhibition. 
                        When updated many times with default the firingrate will be around 4 Hz. 
        :type nu_in: float
        :param e_stim: Determines if the stimulus is part of an evoked stimulus train. 
                Evoked stimuli do not respond to somatodendritic inhibition. 
                So if *e_stim* == True, then *nu* = *nu_in*. 
                Also used when analyzing experimental firing rates (because they already have had autoinhibition)
        :type e_stim: bool
        :param Conc: Concentration of ligands. **Un-necessary unless simulating drugs**. 
                First element is a placeholder, always overwritten by current concentration of dopamine in the relevant compartment. 
                When simulating presence of *N* drugs, *Conc* must be length *N+1*
        :type Conc: array
...
#        # print(Conc)
    ...
    def AnalyticalSteadyState(self, mNU = 4):
        """
        This is a function that calculates analytical values of steady-state DA concentrations 
        and standard deviation of baseline based on the system parameters. 
    ...
    def CreatePhasicFiringRate(self, dt,  Tmax, Generator = 'GraceBunney', Nuaverage = 5,  Nuburst = 20, Tperiod = 1,  Tpre = 0, dtsmooth = 0.1, CV = 1.5):
        """
        This method creates a NU-time series with a repetivite pattern of bursts and pauses - Grace-bunney-style. 
        The NU time series can be given as input to :func:`update`.
        ...
        :type Tmax: float. 
        :param Generator: Selects the type of firing pattern used. Not case sensitive. 
            'GraceBunney' leads to repetitive pattern of burst and pauses. 
            'Gamma' leads to randomly generated bursts and pauses
...
#            # print(spikehist)
            NUphasic = gsmooth(spikehist.astype(float), W,mode = 'wrap')/dt;
...
class TerminalFeedback(receptor):
    """ 
    DA terminal Feedback loop, special case of :class:`receptor`. U
    ses receptor activation to regulate multiplicative gain of DA release from terminals,
...
class SomaFeedback(receptor):
    """ 
    DA somatodendritic Feedback loop, special case of :class:`receptor`.
     Uses receptor activation to regulate additive gain of DA firing rate,
    is evoked by the :class:`DA`-class.
    
    The feedback is similar to `Dreyer and Hounsgaard, 
    J Neurophys, 2013 <https://www.physiology.org/doi/10.1152/jn.00502.2012>`_ 
    and is regulated by parameter alpha
    
    :param alpha: Gain paramter. *alpha* = 0 means no feedback. 
    The larger the value, the stronger the feedback.
    :type alpha: float >= 0
...
class PostSynapticNeuron:
    ...
    :param drugs: instance of :class:`Drug`-class. If none are given it is assumed that dopamine is the only ligand. More than one 
    ...
    def __init__(self, neurontype,  *drugs):
        ...
        if neurontype.lower() == 'd1' :
            self.k_on = 0.01  # SANDRA This is the default in the code t give EC50 = 1000 nm = 1um (D1 adequate)
            self.k_off = 10
            # print(self.k_on)
        elif neurontype.lower() == 'd2':
            self.k_on = 0.2 # SANDRA Changed this to reach the EC50 = 10 nm 
            self.k_off = 2 # SANDRA Changed this to reach the EC50 = 10 nm 
    ...
    def updateNeuron(self, dt, C_DA_ligands, C_other_ligand = 100):
        ...
        self.updateCAMP(dt)
        # SANDRA added the u[date]:------check how the update is carried out
        if self.cAMP >  self.cAMPhigh:
            Kbmax = .01 # rate of bmax update 
            self.updateBmax(Kbmax)
        # --------
  

    def updateBmax(self, Kbmax, cAMP_vector = None, dtsim = 1):
        """
        This method updates Bmax either D1 or D2 MSNs. 
        Bmax is incremented based on current *caMP* compared to the limits *cAMPlow* and *cAMPhigh*. 
        *cAMPoffset* also determines the final rate.  
        We use fast and slow decay methods for 
        Use a vector of cAMP values to batchupdate. For example using :func:`Get_the_cAMP`. 
        
        :param Kbmax: Rate of bmax-updates. 
        Can be different for D1 and D2 MSN's. 
        The real decay rate will also depend on self.cAMPoffset which is set by the neuron. 
        :type Kbmax: float
...
def analyze_spikes_from_file(ToBeAnalyzed, DAsyst, dt = 0.01, synch = 'auto', pre_run = 0,
    ...
    * The method decides the optimal values for Bmax for D1 and D2 neuronal systems. 
    Thus pacemaker firing will always translate into low cAMP in D1 and D2 post neurons 

    :param ToBeAnalyzed: Input firing pattern. Two types of valid inputs: 
        *ToBeAnalyzed* can be * Filename* including full path to experimental data file with time-stamps, as described more below. 
        *ToBeAnalyzed* can also be a numpy array of time-stamps.
    ...
    :param dt: Timestep in simulation (dt = 0.01s by default)
        :type dt: float
    :param synch: FWHM in s for other neurons in ensemble. 
    If set to 'auto', the synch is decided based on average firing rate of the input cell. Default is 'auto'
        :type synch: float
    :param pre_run: Number of seconds to run a totally tonic simulation before input firing pattern kicks in. 
        Very useful to detect changes in the firing pattern. Default is 0.
        :type pre_run: float
    :param tmax: Length of simulation in seconds, default None. If *tmax* is None, tmax will be equal to the last spike in the recording. 
        :type tmax: float
    :param process: Indicate if the function should process the timestamps or not.
         If *True* the method will also include a DA simulation. 
         If *False* the output will only contain the timestams and firing rates of the file. Default is *True*
    :type process: bool
    ...
    if verbose == True:
       print('Finished processing spike times... ')
    ...
    iphasic_on = np.where(tall > pre_run)[0][0] 
    #pre_run >= 0, and tall[0] = dt/2. So guaranteed to get at least one 
    ...
    Results['input_firing_rate'] = NUall
    Results['mean_input_firing_rate'] = mNU
    Results['synch'] = synch
    Results['timeax'] = tall
    ...
    Results['da_Conc_term'] = np.zeros(NUall.size)
    Results['d1_OccfromFile'] = np.zeros(NUall.size)
    Results['d2_OccfromFile'] = np.zeros(NUall.size)  
    Results['d1_AC5fromFile'] = np.zeros(NUall.size)
    Results['d1_cAMPfromFile'] = np.zeros(NUall.size) 
    Results['d2_AC5fromFile'] = np.zeros(NUall.size)
    Results['d2_cAMPfromFile'] = np.zeros(NUall.size)
    ...
    for k in range(NUall.size):
        ...
        Results['da_Conc_term'][k] = Result.da.Conc_DA_term
        Results['d1_OccfromFile'][k] = Result.d1.DA_receptor.occupancy[0]
        Results['d2_OccfromFile'][k] = Result.d2.DA_receptor.occupancy[0]
        Results['d1_AC5fromFile'][k] = Result.d1.AC5()
        Results['d1_cAMPfromFile'][k]= Result.d1.cAMP
        Results['d2_AC5fromFile'][k] = Result.d2.AC5()
        Results['d2_cAMPfromFile'][k] = Result.d2.cAMP

        # Result.d1.OccfromFile[k] = Result.d1.DA_receptor.occupancy[0]
        # Result.d2.OccfromFile[k] = Result.d2.DA_receptor.occupancy[0]
        # Result.d1.AC5fromFile[k]  = Result.d1.AC5()
        # Result.d1.cAMPfromFile[k] = Result.d1.AC5()
        # Result.d2.AC5fromFile[k]  = Result.d2.AC5()
        # Result.d2.cAMPfromFile[k] = Result.d2.cAMP

    if verbose == True:
        print('... done')
    Results['iphasic_on'] = iphasic_on
    Results['phasic_off'] = iphasic_off
    Results['analytical_meanDA'] = mda;
    Results['analytical_stdDA'] = sda;
    Results['meanDAfromFile']  = np.mean(Results['da_Conc_term'][iphasic_on:iphasic_off])
    Results['d1_meanAC5fromFile'] = np.mean(Results['d1_AC5fromFile'][iphasic_on:iphasic_off])
    Results['d2_meanAC5fromFile'] = np.mean(Results['d2_AC5fromFile'][iphasic_on:iphasic_off])
    Results['d1_meancAMPfromFile'] = np.mean(Results['d1_cAMPfromFile'][iphasic_on:iphasic_off])
    Results['d2_meancAMPfromFile'] = np.mean(Results['d2_cAMPfromFile'][iphasic_on:iphasic_off])
    
    if iphasic_on > 0:
        Results['d1_tonic_meanAC5'] = np.mean(Results['d1_AC5fromFile'][:iphasic_on])
        Results['d2_tonic_meanAC5'] = np.mean(Results['d2_AC5fromFile'][:iphasic_on])
        Results['d1_tonic_meancAMP'] = np.mean(Results['d1_cAMPfromFile'][:iphasic_on])
        Results['d2_tonic_meancAMP'] = np.mean(Results['d2_cAMPfromFile'][:iphasic_on])
        Results['dAC5'] = [Results['d1_meanAC5fromFile']/Results['d1_tonic_meanAC5'] , 
                            Results['d2_meanAC5fromFile']/Results['d2_tonic_meanAC5'] ]
...
def udpate_results_dict(Results,simulation_results,max_samp,i):
    ...
    le = min(len(Results['input_firing_rate']),max_samp)
    simulation_results['time_ax'][0:le,i] = Results['timeax'][0:le]
    simulation_results['input_fr'][0:le,i] = Results['input_firing_rate'][0:le]
    simulation_results['da_conc'][0:le,i] = Results['da_Conc_term'][0:le]
    simulation_results['d1_occ'][0:le,i] = Results['d1_OccfromFile'][0:le] 
    simulation_results['d2_occ'][0:le,i] = Results['d2_OccfromFile'][0:le] 
    simulation_results['d1_ac5'][0:le,i] = Results['d1_AC5fromFile'][0:le] 
    simulation_results['d2_ac5'][0:le,i] = Results['d2_AC5fromFile'][0:le] 
    simulation_results['d1_camp'][0:le,i] = Results['d1_cAMPfromFile'][0:le]
    simulation_results['d2_camp'][0:le,i] = Results['d2_cAMPfromFile'][0:le]
    simulation_results['dac5'][:,i] = Results['dAC5']
    simulation_results['analytical_meanDA'][i] = Results['analytical_meanDA']
    simulation_results['analytical_stdDA'][i] = Results['analytical_stdDA']
    simulation_results['meanDA'][i] = Results['meanDAfromFile']
    simulation_results['d1_meanAC5'][i] = Results['d1_meanAC5fromFile']
    simulation_results['d2_meanAC5'][i] = Results['d2_meanAC5fromFile']
    simulation_results['d1_meancAMP'][i] = Results['d1_meancAMPfromFile'] 
    simulation_results['d2_meancAMP'][i] = Results['d2_meanAC5fromFile'] 
    simulation_results['d1_tonic_meanAC5'][i] = Results['d1_tonic_meanAC5'] 
    simulation_results['d2_tonic_meanAC5'][i] = Results['d2_tonic_meanAC5']
    simulation_results['d1_tonic_meancAMP'][i] = Results['d1_tonic_meancAMP'] 
    simulation_results['d2_tonic_meancAMP'][i] = Results['d2_tonic_meancAMP']
    simulation_results['i_phasic'][:,i] = [Results['iphasic_on'], Results['phasic_off']]

    return simulation_results

def initialize_variables_dict(max_samp,n_trials):
    
    simulation_results = dict()
    simulation_results['synch'] = np.zeros((n_trials,))
    simulation_results['mean_input_fr'] = np.zeros((n_trials,))
    simulation_results['time_ax'] = np.zeros((max_samp,n_trials))
    simulation_results['input_fr'] = np.zeros((max_samp,n_trials))
    simulation_results['da_conc'] = np.zeros((max_samp,n_trials))
    simulation_results['d1_occ'] = np.zeros((max_samp,n_trials))
    simulation_results['d2_occ'] = np.zeros((max_samp,n_trials))
    simulation_results['d1_ac5'] = np.zeros((max_samp,n_trials))
    simulation_results['d2_ac5'] = np.zeros((max_samp,n_trials))
    simulation_results['d1_camp'] = np.zeros((max_samp,n_trials))
    simulation_results['d2_camp'] = np.zeros((max_samp,n_trials))
    simulation_results['dac5'] = np.zeros((2,n_trials))
    simulation_results['analytical_meanDA'] = np.zeros((n_trials,))
    simulation_results['analytical_stdDA'] = np.zeros((n_trials,))
    simulation_results['meanDA'] = np.zeros((n_trials,))
    simulation_results['d1_meanAC5'] = np.zeros((n_trials,))
    simulation_results['d2_meanAC5'] = np.zeros((n_trials,))
    simulation_results['d1_meancAMP'] = np.zeros((n_trials,))
    simulation_results['d2_meancAMP'] = np.zeros((n_trials,))
    simulation_results['d1_tonic_meanAC5'] = np.zeros((n_trials,))
    simulation_results['d2_tonic_meanAC5'] = np.zeros((n_trials,))
    simulation_results['d1_tonic_meancAMP'] = np.zeros((n_trials,))
    simulation_results['d2_tonic_meancAMP'] = np.zeros((n_trials,))
    simulation_results['i_phasic'] = np.zeros((2,n_trials))
